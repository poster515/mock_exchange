...
extern "C" {
    ...
    archive::ArchivePublication* archive_pub_create(const char* queue_name, size_t queue_size) {
        spdlog::info("Creating new archive");
    ...
    bool archive_pub_is_ready(archive::ArchivePublication* pub) {
        spdlog::info("Attempting to get archive readiness");
        return pub->is_ready();
    }
    ...
    void* archive_sub_create(const char* queue_name, size_t queue_size) {
